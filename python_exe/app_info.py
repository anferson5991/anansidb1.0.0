...
def extract_playstore_id(url):
    if "id=" in url:
        id = url.split("id=")[1]
        return id
    else:
        return "Invalid URL"
def google_store_app_data(db_path, url):

    id = extract_playstore_id(url)
    print(f'o id eh {id}')
    params = {
    "id": id,                 # app name
    ...
    # Save to 'app_info.json'
    sqliteJsonCreator.create(db_path, app_data)

    # Print confirmation message
    print("Dados do app salvos no sqlite json")
